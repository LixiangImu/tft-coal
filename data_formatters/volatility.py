...
class VolatilityFormatter(GenericDataFormatter):
  ...
  def format_predictions(self, predictions):
    ...
    Args:
      predictions: 模型预测的数据框或字典。

    Returns:
      未标准化预测的数据框。
    """
    print("接收到的预测结果:", predictions)
    ...
    # 保留其他列（如 forecast_time 和 identifier）
    for col in predictions.columns:
        if not col.startswith('t+'):
            output[col] = predictions[col]
    
    print("格式化后的预测结果:", output)
    return output
  ...
  def get_target_names(self):
    """返回目标列的名称。"""
    return [col for col in self._column_definition if col[2] == InputTypes.TARGET]
